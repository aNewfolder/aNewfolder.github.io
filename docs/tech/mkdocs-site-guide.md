...
直到我看到 [LadyGege老师好漂亮好漂亮的个人网站](https://ladygege2494.github.io/)。

在此之前见到的个人博客大多是以课程资源的呈现为主，比如拯救了数届学子的 [savia的外装代脑](https://savia7582.github.io/Exterior/)，比如[咸鱼暄的代码空间](https://xuan-insr.github.io/)，但在看到 LadyGege 老师的网站之后我说，我也想要做一个这样的东西。虽然不知道是否能够像前辈们一样给读者带来如此大的意义，但至少可以以一种半私密的形式记录自己。

于是暑假在家无事，尝试了一下这件事情。欢迎关注[我的网站](https://alight404.top/)。那么我觉得一定还有更多，不管是现在还是未来，正在犹豫着想要做这件事情的同学，所以结合 AI 的帮助写了这篇教程。
...
如果你完全没接触过，建议先花几分钟看一下 [这个教程](https://zju-paradox.top/Latex/)，可以顺便把 Markdown 和 LaTeX 都了解一下，常用工具。

当然，现在 AI 的能力已经足够让我们用自然语言实现任何目的——你完全可以不学前置知识，直接对智能体说"帮我写一篇标题为 xxx 的文章"。但略微掌握一点 Markdown，能让你**自己随手修改一些小地方**时不用事事都麻烦智能体，会方便很多。
...
??? tip "还没有智能体？手把手教你部署（以 Opencode + DeepSeek API 为例）"

    ### 1. 先搞懂两个名词：API 和 API Key
    
    - **API**：你可以把它理解成"程序打电话的号码"。智能体本身没有大脑，它需要通过 API 去调用云端的大模型（比如 DeepSeek），大模型思考完把结果传回来。
    - **API Key**：相当于你打电话用的"账号密码"，是一串以 `sk-` 开头的字符。大模型厂商靠它识别你是谁、从你的余额里扣费。**它只能你自己知道，不要发给任何人、不要贴到公开网站上。**
    ...
    除了上面这种"按量付费"的方式，各大模型厂商还提供 **Coding Plan（编程套餐）**：包月付费，额度很大，而且能用上各家**最强**的模型。比如智谱的 GLM Coding Plan、Kimi、Qwen 等都有类似套餐，一般几十元一个月起。
    ...
    8. 输入 `/models` 回车，选择你想用的模型（比如 `deepseek-v4-flash`）。
...
然后在这个文件夹里启动智能体：打开文件夹，在空白处**右键 → 在终端中打开**，输入 `opencode` 回车。
...
- **想改文章内容** → 找到 `docs/` 里对应的 `.md` 文件，用记事本或任何markdown编辑器打开改。
...
??? note "买不买域名？区别 + 阿里云购买与绑定全流程"

    ### 买和不买有什么区别
    
    - **不买**：你的网址是 `你的用户名.github.io`，免费、完全够用，功能上没有任何区别。
    - **买**：网址变成你自己挑的名字（比如 `alight404.top`），更好看、更像"自己的网站"。便宜的后缀一年只要十几元。
    ...
    3. 点 **加入清单 → 立即结算**。
    4. 结算前需要创建一个 **域名信息模板**（域名持有人信息）：个人用户填真实姓名、身份证号、邮箱，然后按提示完成**实名认证**（信息模板审核官方说 1~5 个工作日，通常很快）。
    ...
    **第一头：阿里云这边做解析**（界面可能与此处不同，不懂如何操作就截图问智能体）
    ...
    2. 点 **添加记录**，添加一条：
       ...
       - 主机记录：`@`
       - 记录值：`你的用户名.github.io`
    3. 点 **添加记录**，添加 4 条 **A 记录**，记录值分别填 GitHub 的四个 IP：`185.199.108.153`、`185.199.109.153`、`185.199.110.153`、`185.199.111.153`。
